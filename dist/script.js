...
function () {
  ...
  _createClass(Difference, [{
    ...
    value: function init() {
      try {
        this.hideItems(this.oldItems); //чтобы не было повторения кода
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      ...
      function createMask(event) {
        //this использовать можно, но очень внимательно!
        //принадлежит функции, если ее неправильно использовать, то он тоже потеряется
  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function () {
          ...
          if (btn.closest('.feed__item-active')) {
            _this.feed.style.visibility = 'hidden';
          }
        });
      });
    }
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        console.log('s');
        _this2.feedSlider.style.display = 'block';
        _this2.overlay.style.display = 'none';
        _this2.feed.style.visibility = 'visible';

        _this2.player.stopVideo();

        _this2.player.destroy(); //мой более грубый вариант
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        ...
        width: '100%',
        videoId: "".concat(url),
        //будем подгружать реальный уникальный идентификатор! 
        events: {
